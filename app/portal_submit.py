...
class PortalSubmitRequest(BaseModel):
    ...
    # R1: explicit declarations (defaults are safe / non-breaking)
    ai_assisted: bool = Field(default=False, description="User declares AI assistance was used.")
    user_confirmed_review: bool = Field(
        default=True,
        description="User confirms they reviewed the AI-assisted output.",
    )
...
class SubmissionItem(BaseModel):
    ...
    # Useful in list views (optional, but low-risk to include)
...
def override_submission(
    ...
    """
    Marks an existing submission as manually overridden/reviewed.
    Metadata-only.
    """
    clinic_id = getattr(request.state, "clinic_id", None)
    clinic_user_id = getattr(request.state, "clinic_user_id", None)
    ...
    if not row:
        # Either not found, or not owned by this user in this clinic
        raise HTTPException(status_code=404, detail="receipt not found")
