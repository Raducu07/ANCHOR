...
| 6 | M6.13 ambient governance shell (extreme caution) | **Committed** | Metadata-only ambient_governance_events (RLS+FORCE; no content-capable columns), review gate (decision categories only), validation-only ingestion interface (no vendor adapter), all endpoints 503 unless ANCHOR_AMBIENT_GOVERNANCE_ENABLED. Clinical-content boundary spec DRAFT in docs/product. 15 tests passed. |
| 7 | Frontend roadmap work | **Not executable in this workspace** — the portal frontend repo is not present. New backend contracts for a later frontend session: `/v1/portal/onboarding/*`, `/v1/learn/checks|paths|renewals|renewal-policy`, `/v1/portal/billing/state`, `/v1/portal/sustainability/*`, `/v1/portal/ambient/*` (flag-gated). | — |
