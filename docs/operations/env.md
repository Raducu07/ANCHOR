...
| `ANCHOR_GENERATION_PROVIDER` | Selects the generation provider adapter in `app/assistant_provider.py`. | unset → `anthropic` | **Prod permits `anthropic` only** — any other value is refused at resolution time and the Workspace orchestrator falls back deterministically. This is M6.12 code-wise completion under the 2026-07-05 founder decision record, not a public provider-choice capability; do not document or claim provider choice as a capability. |
| `OPENAI_API_KEY` | Credential for the gated OpenAI adapter (stdlib HTTPS, no SDK). Read **only** inside `app/assistant_provider.py` (test-enforced). | unset → adapter fails closed (`openai_api_key_missing` config error → deterministic fallback) | **Non-prod only in practice**: the adapter can never be selected in prod regardless of this key. Enabling OpenAI anywhere beyond local experimentation is a founder decision with subprocessor/legal implications. Treat as a high-sensitivity secret. |
| `ANCHOR_OPENAI_MODEL` | Model id used by the gated OpenAI adapter when selected. | `gpt-4o-mini` | Change only via deliberate version review. Irrelevant while the adapter is unselected. |

`OPENAI_API_KEY` / `ANCHOR_OPENAI_MODEL` are read **only** by the gated adapter in `app/assistant_provider.py` (enforced by `tests/test_assistant_provider.py::test_openai_env_vars_read_only_in_assistant_provider`). The OpenAI adapter cannot be selected in production and is inert unless explicitly configured in a non-prod shell. Anthropic remains the only production-permitted provider. ANCHOR is **architected for** vendor-neutrality, not vendor-neutral as a public current-state capability, and no public copy may claim otherwise. Output safety validation is provider-independent: every live draft passes the same post-output validator regardless of adapter.
